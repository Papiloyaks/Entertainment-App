  ...
  const [pageNumber, setpageNumber] = useState(0)
    ...
    const navigate = useNavigate()
      ...
      const detailsPage = (e,mediaType) => {
        ...
        <h1 className='ms-5 my-5 text-xl text-amber-300'>Similar Movies:</h1>
        <div className='grid lg:grid-cols-5 grid-cols-3 gap-10 mx-5'>
        {similar.length > 0 ?
              similar.slice(pageVisited, pageVisited + moviePage).map((item,i)=>(
                <div onClick={()=>detailsPage(item.id, item.media_type)} key={i} className=''>
                    <img src={`${imgBaseUrl}/original/${item.poster_path}`} className='w-[200px] h-[150px] hover:scale-110 rounded' alt="" />
                    <div className='text-center text-white p-2'>{!myId.mediaType? `${item.title}`: `${item.name}`}</div>
                </div>
              )) 
            :
            <div>
